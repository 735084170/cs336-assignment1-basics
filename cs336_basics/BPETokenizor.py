    ...
    for i in index_set:
        ...
        while j < len(pretoken):
            if (j < len(pretoken)-1) and ((pretoken[j], pretoken[j+1]) == max_pair):
                ...
                if new_pretoken != []:
                    counts[(new_pretoken[-1], pretoken[j])] -= 1
                    counts[(new_pretoken[-1], new_index)] += 1
                    index_dict[(new_pretoken[-1], new_index)].add(i)

                if j < len(pretoken)-2:
                    counts[(pretoken[j+1], pretoken[j+2])] -= 1
                    counts[(new_index, pretoken[j+2])] += 1
                    index_dict[(new_index, pretoken[j+2])].add(i)
