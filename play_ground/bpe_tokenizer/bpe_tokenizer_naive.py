...
def find_special_tokens(file_path):
    with open(file_path, 'r') as f:
        text = f. read()
    parttern = r'<\|(.*?)\|>'
    return list(set(re.findall(parttern, text)))
...
def update_token_group_list(token_group_list:list, max_vab:tuple[bytes, bytes]):
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    for chunk in chunks:
        ...
        token_group_list_batch.append(token_group_list)
    ...
    for _ in tqdm(range(vocab_size-len(vocab))):
        ...
        with multiprocessing.Pool() as pool:
            results = pool.map(count_from_chunks, token_group_list_batch)
            gather_merge_count = {}
            for count_dic in results:
                for k, v in count_dic.items():
                    if k not in gather_merge_count:
                        gather_merge_count[k] = v
                    else:
                        gather_merge_count[k] += v
        # get the new merged vocab
        max_count = 0
        max_vab = (bytes([0]),bytes([0]))
        for k, v in gather_merge_count.items():
            if v > max_count or (v == max_count and k > max_vab):
                max_count = v
                max_vab = k
        vocab[len(vocab)] = max_vab[0] + max_vab[1]
        print(max_vab)
        merges.append(max_vab)

        # update base on the new vocab
        with multiprocessing.Pool() as pool:
            args_for_starmap = zip(token_group_list_batch, repeat(max_vab))
            results = pool.starmap(update_token_group_list, args_for_starmap)
            token_group_list_batch = results
