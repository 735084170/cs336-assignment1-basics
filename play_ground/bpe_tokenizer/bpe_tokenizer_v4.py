...
def pre_tokenization(text: str, special_tokens: list[str]) -> list[bytes]:
    parts = split_by_special_tokens(text, special_tokens)
    pat = r"""'(?:[sdmt]|ll|ve|re)| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+"""
    
    blocks = []
    for part in parts:
        blocks.extend(re.findall(pat, part))
    bytes_blocks = [block.encode("utf-8") for block in blocks]
    tokens = [block for block in bytes_blocks]
    return tokens
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    with multiprocessing.Pool() as pool:
        results = pool.map(pre_tokenization, chunks)
    pretokens = [token for tokens in results for token in tokens]
